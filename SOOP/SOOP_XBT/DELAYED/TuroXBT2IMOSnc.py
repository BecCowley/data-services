...
def extract_turo_data(nco, n, crid, callsign, ship_IMO, ship_name, line_info, raw_netCDF_file):
    ...
    for index, row in varslist.iterrows():
        ...
        if vname in ['TIME','PROBE_manufacture_date_YYYYMMDD', 'SAMPLE_TIME']:
            if vname == 'SAMPLE_TIME':
                ...
                if vname == 'PROBE_manufacture_date_YYYYMMDD':
                    ...
                    if data is None or test:
                        # data is not applicable as it is a test canister, fill the profile['PROBE_manufacture_date_YYYYMMDD'] with None
                        dfprofile['PROBE_manufacture_date_YYYYMMDD'] = None
                    else:
                        if type(data) == str or data is None:
                            ...
                            dfprofile['PROBE_manufacture_date_YYYYMMDD'] = pd.to_datetime(data)
                else:
